...
export class TransformHandle {
  ...
  public getStyleLeft(): string {
    return `calc(${this.data.vectorRounded.x * 50 + 50}% - ${
      0.5 * this.getSize()
    }px)`;
  }
  public getStyleTop(): string {
    return `calc(${this.data.vectorRounded.y * 50 + 50}% - ${
      0.5 * this.getSize()
    }px)`;
  }
  ...
  public isOnTopLefDiagonal(): boolean {
    return (
      this.data.direction === Direction.TOPLEFT ||
      this.data.direction === Direction.BOTTOMRIGHT
    );
  }
  public isOnTopRightDiagonal(): boolean {
    return (
      this.data.direction === Direction.TOPRIGHT ||
      this.data.direction === Direction.BOTTOMLEFT
    );
  }
}
